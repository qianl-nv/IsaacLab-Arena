...
def add_script_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--dump_spheres_dir", type=str, default=None, help="Directory to dump CuRobo collision spheres."
    )
...
def _execute_plan(
    ...
    for pose in planned_poses:
        ...
        _update_ee_visualizer(
            ee_visualizer,
            _get_current_eef_pose(env, planner, env_id),
            robot_base_pos,
        )
    ...
    for step in range(max_converge_steps):
        if pos_err < converge_pos_threshold and rot_err < converge_rot_threshold:
            print(
                f"[CONVERGE] Reached final waypoint in {step} extra steps "
    ...
    print(
        f"[CONVERGE] Did not fully converge after {max_converge_steps} steps "
        f"(pos_err={pos_err:.4f}m, rot_err={rot_err:.4f}rad)"
    )
...
# Neel: This is a hack to fix the planner's object sync so poses are in robot-base frame, not env-origin frame.
# TODO (Neel): Fix this later in the upstream cuRobo planner interface
def _fix_planner_object_sync_frame(planner) -> None:
    """Replace the planner's object sync so poses are in robot-base frame, not env-origin frame."""
    import types
    ...
    def _sync_robot_base_frame(self):
        ...
        for object_name, object_path in object_mappings.items():
            ...
            quat_robot = math_utils.quat_mul(
                robot_quat_inv.unsqueeze(0),
...
def main() -> None:
    ...
    with SimulationAppContext(args_cli):
        ...
        _fix_planner_object_sync_frame(planner)
